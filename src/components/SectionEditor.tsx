...
import { existingTexts } from '../lib/checkpoints';
import { SuggestCheckpoints } from './SuggestCheckpoints';
...
/**
 * The section's own suggest panel.
 *
 * The brief starts as the section's title and description joined together, and
 * is then the admin's to reword: the sentence that gets the best suggestions is
 * rarely the one that reads best as a heading. The new-checklist form works the
 * other way round — see `ChecklistNewScreen`.
 */
function SectionSuggest({
  ...
  return (
    <SuggestCheckpoints
      ...
      existing={existingTexts(section.questions)}
      isAdmin={!readOnly}
      intro="Describe what this section covers. Suggestions are added one at a time, and nothing is saved until you save the checklist."
      onAdd={onAdd}
    />
  );
}
